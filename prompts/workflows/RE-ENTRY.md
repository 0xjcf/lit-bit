...
2. Open the active phase checklist (look at the phase named in the summary, e.g. prompts/phases/02-development/00_checklist.md) and:
...
3. Check for a task decomposition file for the **current phase** (identified in step 1) in the `prompts/decomposition/` directory (e.g., `NN_<phase-name-slug>_tasks.md`).
   ...
   b. If it **does not exist**, propose creating it by breaking down the items from the current phase's `00_checklist.md` (from step 2) into actionable sub-tasks. List these proposed sub-tasks. *(This new decomposition file should be the first task to complete if missing).* 
